...
# average age of patients in the training set, ClaimsTrain
mean(ClaimsTrain$age)
#sum(ClaimsTrain$age)/nrow(ClaimsTrain)
...
NPenaltyMatrix = matrix(c(0,2,4,6,8), byrow=FALSE, nrow=5)
NPenaltyMatrix

as.matrix(table(ClaimsTest$bucket2009)) * NPenaltyMatrix

sum(as.matrix(table(ClaimsTest$bucket2009)) * NPenaltyMatrix)/nrow(ClaimsTest)
# 1.044301
